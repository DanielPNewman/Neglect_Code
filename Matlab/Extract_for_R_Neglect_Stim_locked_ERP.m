...
%     t=t(find(t==-200):find(t==1500));
    ...
    for trial=1:length(allTrials) % get rid of last trigger?
        ...
        if ismember(subject_folder{s},subject_folder_Neglect)
            master_matrix_R(current:current+(length(t)-1),2) = 1; % (1=Neglect)
        elseif ismember(subject_folder{s},subject_folder_Control)
        ...
        %% 14. GAZE_X:
        if ismember(subject_folder{s},EyeTracker)
         master_matrix_R(current:current+(length(t)-1),14)=GAZE_X(:,trial);    
        else
         master_matrix_R(current:current+(length(t)-1),14)=0;       
        end
        %% 15. GAZE_Y:
        if ismember(subject_folder{s},EyeTracker)
         master_matrix_R(current:current+(length(t)-1),15)=GAZE_Y(:,trial);
        else
         master_matrix_R(current:current+(length(t)-1),15)=0;
        end
...
csvwrite (['master_matrix_R_Stim_locked_ERP_Neglect.csv'],master_matrix_R)

...
for s=1:length(allsubj)
    ...
    if ismember(subject_folder{s},Monash_Stationary_EEG)
        ...
        %     LH_ROI=LH_ROI_s;
        %     RH_ROI=RH_ROI_s;
        %same alpha ROI for all participants, uncomment the above if you want
        %individualised ROIs
    LH_ROI_s=[23,24,28,29,56,57,60,61];
    RH_ROI_s=[26,27,31,32,58,59,63,64];
     elseif ismember(subject_folder{s},Monash_Portable_EEG)
        ...
        %same alpha ROI for all participants, uncomment the above if you want
        %individualised ROIs
    LH_ROI_s=[15,14,16,11,12,10];
    RH_ROI_s=[18,20,21,19,22,23];
    else
...
%         load([path 'Monash_Participants\' subject_folder{s} '\' 'avN2c_ParticipantLevel_peak_amp_index.mat'])
%         load([path 'Monash_Participants\' subject_folder{s} '\' 'avN2i_ParticipantLevel_peak_amp_index.mat'])
%         load([path 'Monash_Participants\' subject_folder{s} '\' 'ROIs.mat'])
...
%         master_matrix_R(total_numtr,14)=Threshold_cohLevel; 
        ...
        master_matrix_R(total_numtr,20)=0;    
        end
        ...
        master_matrix_R(total_numtr,21)=0;    
        end
       %% 22. Group (Neglect (1) vs. Control(2)) 
       master_matrix_R(total_numtr,22) = group;
        
%% Just dan exploring :        
%         %% 23. N2c Amp (using PARTICIPANT LEVEL AVERAGE to define N2c measurement window):
...
%         master_matrix_R(total_numtr,23)=mean(mean(erp(ch_N2c(TargetSide,:),(avN2c_ParticipantLevel_peak_amp_index_s(TargetSide))-window:avN2c_ParticipantLevel_peak_amp_index_s(TargetSide)+window,trial),1));
%         %% 24. N2i Amp (using PARTICIPANT LEVEL AVERAGE to define N2i measurement window):
%         master_matrix_R(total_numtr,24)=mean(mean(erp(ch_LR(TargetSide,:),avN2c_ParticipantLevel_peak_amp_index_s(TargetSide)-window:avN2c_ParticipantLevel_peak_amp_index_s(TargetSide)+window,trial),1));
%         %% 25. Post-target Alpha Power Left Hemi:
%         master_matrix_R(total_numtr,25)=squeeze(mean(mean(Alpha([LH_ROI],find(Alpha_smooth_time==300):find(Alpha_smooth_time==1000),trial),1),2));
%         %% 26. Post-target Alpha Power Right Hemi:
%         master_matrix_R(total_numtr,26)=squeeze(mean(mean(Alpha([RH_ROI],find(Alpha_smooth_time==300):find(Alpha_smooth_time==1000),trial),1),2));
%         %% 27. CPP half peak latency:
...
%         if half_max_peak<0
%             master_matrix_R(total_numtr,27)=0;
%         elseif isempty(half_max_peak_index)
%                 master_matrix_R(total_numtr,27)=0;
%         else 
%         master_matrix_R(total_numtr,27)=t_crop(half_max_peak_index);
%         end
%         
%         %% 28. N2c peak latency:
...
%             master_matrix_R(total_numtr,28)= 0; %%DN: make it 0 instead of NaN, will remove these in R
%         else
%             % it's good! add it in.
%             master_matrix_R(total_numtr,28)=t_crop(N2pc_min_ind);  %N2c_peak_latencies(trial)= t(N2pc_min_ind);
%         end
%        %% 29. Respose locked CPP slope: (just fitting a straight line, like in Kelly and O'Connel J.Neuro, but on trial-by-trial basis)
...
%             master_matrix_R(total_numtr,29) = coef(1); %slope
...
csvwrite ('master_matrix_R_Neglect.csv',master_matrix_R)
cell2csv ('ID_vector_Neglect.csv',ID_vector)

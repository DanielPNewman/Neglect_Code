...
allsubj=   {'HN999','HN998','HN996', 'HN995', 'HN994','HN993','HN992','HN990','HN989','HN988','HN987','HN986','HN985','HN983','HN982','HN981','HN980','HN978','HN977','HN976','HN975','HN974','HN973','HN972','HN971','HN970','HN969','HN968',...
    'HN899','HN898','HN897', 'HN896', 'HN895', 'HN894','HN893','HN892', 'HN891', 'HN890','HN889','HN888','HN886','HN885','HN884','HN883','HN882','HN881','HN880','HN879','HN878','HN877','HN876','HN875','HN874','HN873','HN872','HN871','HN870','HN868','HN867'};
duds = [21, 52]; % 'HN975'(21)has an invalid right target CPP onset    ; 'HN875'(52) is an RT outlier (very slow RTs);
single_participants = [];
file_start = 1;

Index=(strfind(subject_folder,'HN875'));
...
for s=file_start:length(allsubj)
    ...
    group_path={'Healthy_Older\','Healthy_Younger\'};
...
figure
counter=0;
scalp_plot_times=[150, 350]; %in ms
for group=1:2
    for targetside=1:2
        ...
        set(gca,'FontSize',16);
        title([Older_Younger{group} Patchside{targetside} num2str(scalp_plot_times(1)) '-' num2str(scalp_plot_times(2)) 'ms']);
    end
end
    ...
    for side=1:2
        for channels = 1:2 %ERP_cond(Subject, channel, samples, patch,motion i,group)
            ...
            h(counter) = plot(t(find(t==-100):find(t==500)),avERP_plot,'Color',colors3{counter},'LineStyle',line_styles3{counter}, ...
                'Linewidth',4); hold on;
            set(gca, 'XLim', [-100 500],'YLim', [-4 2])
            line(xlim,[0,0],'Color','k');
            line([0,0],ylim,'Color','k');
            counter = counter+1;
        end
    end
    set(gca,'FontSize',20,'xlim',[-100,500],'xtick',[-100,0:100:500],'ylim',[-4,2],'ytick',[-3:1:2]);
...
N2cN2i_latency_ByTargetSide = [avN2c_peak_amp_index_t,avN2i_peak_amp_index_t]; %(LeftTargetN2c_latency, RightTargetN2c_latency, LeftTargetN2i_latency, RightTargetN2i_latency)
 ...
 %% Extract N2c and N2i amplitude, using amplitude latency window based on EACH GROUP'S GRAND AVERAGE N2c/i:
...
for s = 1:size(allsubj,2)
    ...
    for TargetSide=1:2
        ...
        avN2c=squeeze(mean(mean(mean(mean(mean(mean(ERP_cond(ERP_cond(:,1,1,1,1,1,group)~=0,ch_N2c(TargetSide,:),:,:,:,:,group),5),4),2),1),6),7)); %(sample)
        avN2i=squeeze(mean(mean(mean(mean(mean(mean(ERP_cond(ERP_cond(:,1,1,1,1,1,group)~=0,ch_N2i(TargetSide,:),:,:,:,:,group),5),4),2),1),6),7));
        avN2c_peak_amp=min(avN2c(find(t==150):find(t==400)));
        avN2i_peak_amp=min(avN2i(find(t==150):find(t==400)));
        avN2c_peak_amp_index(s,TargetSide)=find(avN2c==avN2c_peak_amp);%Find Left target max peak latency for N2c
        avN2i_peak_amp_index(s,TargetSide)=find(avN2i==avN2i_peak_amp);%Find Left target max peak latency for N2i
...
figure
counter=0;
scalp_plot_times=[500, 700;  %in ms for Older
                  350, 550]; %in ms for Younger
for group=1:2
        counter=counter+1; %ERP_cond(Subject, channel, samples, patch,motion, i,group)
        temp=squeeze(mean(mean(mean(mean(mean(ERP_cond(ERP_cond(:,1,1,1,1,1,group)~=0,:,:,:,:,:,group),5),1),4),6),7));
        subplot(1,2,counter)
        topoplot(mean(temp(:,find(t_crop==scalp_plot_times(group,1)):find(t_crop==scalp_plot_times(group,2))),2),chanlocs,'electrodes','numbers','plotchans',1:64); %colorbar
        set(gca,'FontSize',16);
        title([Older_Younger{group} num2str(scalp_plot_times(group,1)) '-' num2str(scalp_plot_times(group,2)) 'ms']);
end


%% Plot stim-locked CPP by TargetSide:
labels = {'Older Left target','Older Right Target','Younger Left Target','Younger Right Target'};
colors4 = {'b' 'r' 'b' 'r'};
figure
counter=0;
for group=1:2
...
set(gca,'FontSize',20,'xlim',[-100,1000],'xtick',[-100,0:100:1000],'ylim',[-1,8],'ytick',[-1:1:8]);
ylabel('Amplitude (\muVolts)','FontName','Arial','FontSize',20)
xlabel('Time (ms)','FontName','Arial','FontSize',20)
...
patch_ipsi_contra = {'Older Left target','Older Right Target','Younger Left Target','Younger Right Target'};
line_styles4={ '--' '-'  '--' '-'};
...
for group=1:2
    for targetside=1:2
        ...
        title(['Alpha Desync. ' Older_Younger{group} Patchside{targetside} num2str(scalp_plot_times(1)) '-' num2str(scalp_plot_times(2)) 'ms']);
    end
end
...
for group=1:2
    ...
    title(['Alpha Left minus Right Target ' Older_Younger{group} num2str(scalp_plot_times(1)) '-' num2str(scalp_plot_times(2)) 'ms']);
end


%% Try alpha Left minus Right Target alpha Topoplot
%Alpha_cond (Subject, channel, samples, patch, i, group)
Group_tag = {'Older','Younger'};
for group=1:2
...
participant_level(:,5:8)=N2cN2i_amp_ByTargetSide_GrandAverage; %N2 amplitude using window based on EACH GROUP's GRAND AVERAGE N2c/i - Contra LeftTarget; Contra RightTarget; Ipsi LeftTarget; Ipsi RightTarget;
...
participant_level(:,13:14)=CPP_patch_onsets; %Stim. locked CPP onset
...
participant_level(:,19:20)=CPP_slope; %LeftTarget, RightTarget

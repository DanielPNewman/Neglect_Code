...
subject_folder = {'HN018','James'};
allsubj=   {'HN018', 'PortableJamesTest'};
...
%%
subject_folder_Neglect = {'HN018'};
allsubj_Neglect = {'HN018'};

subject_folder_Controls = {'James'};
allsubj_Controls = {'PortableJamesTest'};
%%
...
CSD=0; %Use Current Source Density transformed erp? 1=yes, 0=no NOTE this does CSD only for the ERPs, not Alpha
...
tester = zeros(numch,1);
figure
topoplot(tester,chanlocs,'maplimits', ...
    [min(tester)  max(tester)],'electrodes','labels','plotchans',1:numch);
figure
topoplot(tester,chanlocs,'maplimits', ...
    [min(tester)  max(tester)],'electrodes','numbers','plotchans',1:numch);
...
%     
    ...
    clear conds_erp conds_RT conds_Alpha_pre conds_Alpha_post CPPs
    for patch = 1:2
        for motion = 1:2;
            for i = 1:3
        ...
        if ~isempty(onsetp05)
            ...
            disp([allsubj{s},': bugger NOTE ABLE TO DETECT CPP ONSET']) %Some neglect patients wont have CPP onset for left targets
...
for s=file_start:length(allsubj)
    ...
    LH_elec=[15,14,16,11,12,10];
    RH_elec=[18,20,21,19,22,23];
    ...
    LH_ROI(s,:)= LH_elec(temp2(4:6)); %DN: pick the 4 Left Hemisphere parieto-occipito that show the highest left_minus_right values (5:8)
    LH_ROI_s =  LH_ROI(s,:);
    
    RH_desync(s,:)= left_minus_right_per_subject(s,RH_elec);
    [temp1,temp2] = sort(RH_desync(s,:));
    RH_ROI(s,:)= RH_elec(temp2(1:3)); %DN: pick the 4 Right Hemisphere parieto-occipito that show the lowest left_minus_right values (1:4)
    RH_ROI_s =  RH_ROI(s,:);
...
for i = find(t_crop==0):20:find(t_crop==1000)-20
    ...
    topoplot(mean(ERP_LeftTarget(:,i:i+19,:),2),chanlocs,'maplimits', ...
        [min(min(ERP_LeftTarget(1:numch,find(t_crop==0):find(t_crop==1000),:)))  max(max(ERP_LeftTarget(1:numch,find(t_crop==0):find(t_crop==1000),:)))],'electrodes','numbers','plotchans',1:numch);
...
for i = find(t_crop==0):20:find(t_crop==1000)-20
    ...
    topoplot(mean(ERP_RightTarget(:,i:i+19,:),2),chanlocs,'maplimits', ...
        [min(min(ERP_RightTarget(1:numch,find(t_crop==0):find(t_crop==800),:)))  max(max(ERP_RightTarget(1:numch,find(t_crop==0):find(t_crop==800),:)))],'electrodes','numbers','plotchans',1:numch);
    ...
    for group=1:2
        for targetside=1:2
            ...
            topoplot(mean(temp(:,find(t_crop==scalp_plot_times(1)):find(t_crop==scalp_plot_times(2))),2),chanlocs,'maplimits', ...
                [-4  1],'electrodes','numbers','plotchans',1:numch); %colorbar
            set(gca,'FontSize',16);
        ...
        for targetside=1:2
            ...
            subplot(2,2,counter)
            topoplot(mean(temp(:,find(t_crop==scalp_plot_times(1)):find(t_crop==scalp_plot_times(2))),2),chanlocs,'maplimits', ...
                [-40  5],'electrodes','numbers','plotchans',1:numch); %colorbar
            set(gca,'FontSize',16);
            title([Neglect_Controls{group} Patchside{targetside} num2str(scalp_plot_times(1)) '-' num2str(scalp_plot_times(2)) 'ms']);
        end
        end
...
figure
 counter=0;
        for targetside=1:2
            ...
            subplot(2,1,counter)
            topoplot(mean(temp(:,find(t_crop==scalp_plot_times(1)):find(t_crop==scalp_plot_times(2))),2),chanlocs,'maplimits', ...
                [-40  25],'electrodes','off','plotchans',1:numch); %colorbar
            set(gca,'FontSize',16);
 ...
 figure
            ...
            topoplot(mean(temp(:,find(t_crop==scalp_plot_times(1)):find(t_crop==scalp_plot_times(2))),2),chanlocs,'maplimits', ...
                [-25  5],'electrodes','off','plotchans',1:numch); %colorbar
            set(gca,'FontSize',16);
 ...
 figure
            ...
            topoplot(mean(temp(:,find(t_crop==scalp_plot_times(1)):find(t_crop==scalp_plot_times(2))),2),chanlocs,...
                'electrodes','off','plotchans',1:numch); %colorbar
            set(gca,'FontSize',16);
             title([num2str(scalp_plot_times(1)) '-' num2str(scalp_plot_times(2)) 'ms']);
...
for group=1:2
        ...
        subplot(1,2,counter)
        topoplot(mean(temp(:,find(t_crop==scalp_plot_times(group,1)):find(t_crop==scalp_plot_times(group,2))),2),chanlocs,'electrodes','numbers','plotchans',1:numch, 'maplimits',[-1  8]); %colorbar
        set(gca,'FontSize',16);
...
for group=1:2
        ...
        temp=squeeze(mean(mean(mean(mean(mean(ERP_cond(ERP_cond(:,1,1,1,1,1,group)~=0,:,:,:,:,:,group),5),1),4),6),7));
        subplot(1,2,counter)
        topoplot(mean(temp(:,find(t_crop==scalp_plot_times(group,1)):find(t_crop==scalp_plot_times(group,2))),2),chanlocs,'electrodes','numbers','plotchans',1:numch, 'maplimits',[-40  40]); %colorbar
        set(gca,'FontSize',16);
        title([Neglect_Controls{group} num2str(scalp_plot_times(group,1)) '-' num2str(scalp_plot_times(group,2)) 'ms']);
end    
end
...
figure
        ...
        topoplot(mean(temp(:,find(t_crop==scalp_plot_times(1,1)):find(t_crop==scalp_plot_times(1,2))),2),chanlocs,'electrodes','off','plotchans',1:numch); %colorbar
...
left_hemi_ROI = [14,15,16];
right_hemi_ROI = [18,19,20];
alpha_ch=[left_hemi_ROI;right_hemi_ROI];
...
csvwrite (['participant_level_matrix_Neglect_32chPortable.csv'],participant_level)

subject_folder=subject_folder';
cell2csv ('IDs_Neglect_32chPortable.csv',subject_folder)
